import { UserCircle} from '~/components/user-circle';
import type {Profile, Kudo as IKudo  } from "@prisma/client";
import { colorMap, backgroundColorMap, emojiMap} from '~/utils/constants';

export function Kudo({ profile, kudo }: {profile: Profile; kudo: Partial<IKudo>}) {
    return (
        <div
            className={`flex ${
                backgroundColorMap[kudo.style?.backgroundColour || 'RED']
            } p-4 rounded-xl w-full gap-x-2 relative`}
            >
        <div>
            <UserCircle profile={profile} className="h-16 w-16"/>
        </div>
        <div className='flex flex-col'>
            <p className={`${colorMap[kudo.style?.textColour || 'WHITE']} font-bold text-lg whitespace-pre-wap break-all`}>
                {profile.firstName} {profile.lastName}
            </p>
            <p className={`${colorMap[kudo.style?.textColour || 'WHITE']} whitespace-prep-wrap break-all`}>{kudo.message}</p>
        </div>
            {emojiMap[kudo.style?.emoji || 'THUMBSUP']}
        </div>
    )
}